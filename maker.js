...
const translations = {
  en: {
    ...
    botInfo: (username, createdAt, template) =>
      `🤖 @${username}\nCreated: ${createdAt}\nTemplate: ${template}\n\n`,
  ...
  fa: {
    ...
    english: 'انگلیسی',
    persian: 'فارسی',
    welcome: 'به سازنده ربات خوش آمدید! از دکمه‌های زیر برای ایجاد و مدیریت ربات‌های تلگرام خود استفاده کنید.',
    mainMenu: {
      createBot: '🛠 ساخت ربات',
...
makerBot.start(async (ctx) => {
  ...
  try {
    ...
    if (user && user.isBlocked) {
      ...
      ctx.reply(translations[lang].banned);
      return;
    }
  ...
  try {
    ...
    if (isVip) {
      ...
      if (botLimit > 0 && userBotCount >= botLimit) {
        ctx.reply(translations[lang].botLimitReached(botLimit), getMainMenu(lang));
        return;
      }
    ...
    if (botMode.mode === 'lock') {
      ctx.reply(translations[lang].lockMode, getMainMenu(lang));
      return;
    }
    ...
    ctx.reply(translations[lang].error, getMainMenu(lang));
  }
});
  ...
  try {
    ...
    if (userBots.length === 0) {
      ...
      userBots.forEach((bot) => {
        ...
        message += translations[lang].botInfo(bot.username, createdAt, bot.template);
      });
    }
    ctx.reply(message, getMainMenu(lang));
  } catch (error) {
...
makerBot.command('panel', async (ctx) => {
  ...
  if (userId !== OWNER_ID) {
    const user = await User.findOne({ userId });
    const lang = user?.language || 'en';
    ctx.reply(translations[lang].unauthorized);
    return;
  }
  ...
  try {
    ...
    ctx.reply(translations['en'].error, getMainMenu('en'));
  }
});
...
makerBot.on('callback_query', async (ctx) => {
  ...
  try {
    const user = await User.findOne({ userId });
    if (!user) {
      ctx.reply(translations['en'].startBot);
      return;
    }
    ...
    if (callbackData === 'lang_en') {
      ...
      if (user.step === 'choose_language') {
        ...
        await makerBot.telegram.sendMessage(chatId, translations['en'].joinMessage, {
          reply_markup: {
            inline_keyboard: inlineKeyboard,
          },
        });
        ...
        await ctx.reply(translations['en'].welcome, getMainMenu('en'));
      }
      ...
      if (user.step === 'choose_language') {
        ...
        await makerBot.telegram.sendMessage(chatId, translations['fa'].joinMessage, {
          reply_markup: {
            inline_keyboard: inlineKeyboard,
          },
        });
        ...
        await ctx.reply(translations['fa'].welcome, getMainMenu('fa'));
      }
    } else if (callbackData === 'joined') {
      await ctx.answerCbQuery(callbackQueryId, translations[lang].thankYou);
      await ctx.telegram.sendMessage(chatId, translations[lang].welcome, getMainMenu(lang));
    }
  } catch (error) {
...
makerBot.on('text', async (ctx) => {
  ...
  try {
    const user = await User.findOne({ userId });
    if (!user) {
      ctx.reply(translations['en'].startBot, getMainMenu('en'));
      return;
    }
    ...
    if (userId === OWNER_ID && user.adminState === 'admin_panel') {
      ...
      if (!botInfo) {
        ctx.reply(translations[lang].invalidToken, backKeyboard);
        return;
      }

      const existingBot = await Bot.findOne({ token: text });
      if (existingBot) {
        ctx.reply(translations[lang].tokenInUse, getMainMenu(lang));
        await User.findOneAndUpdate({ userId }, { step: 'none' });
        return;
      }
      ...
      if (!webhookSet) {
        ctx.reply(translations[lang].webhookFailed, getMainMenu(lang));
        await User.findOneAndUpdate({ userId }, { step: 'none' });
        return;
      }
      ...
      if (!bot) {
        ctx.reply(translations[lang].botNotFound, getMainMenu(lang));
        await User.findOneAndUpdate({ userId }, { step: 'none' });
        return;
      }
