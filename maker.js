...
const ownerAdminPanel = {
  reply_markup: {
    keyboard: [
      ...
      [{ text: '🔧 Bot Mode' }],
      [{ text: '👑 Add VIP User' }],
      [{ text: '🔗 Set Channel URL' }], // New button
      [{ text: '↩️ Back' }],
    ],
    resize_keyboard: true,
  },
...
makerBot.on('text', async (ctx) => {
  ...
  try {
    ...
    if (userId === OWNER_ID && user.adminState === 'admin_panel') {
      ...
      // Update all created bots' default URLs
      await ChannelUrl.updateMany({}, { $set: { url: correctedUrl } });

      ctx.reply(`✅ Created URL has been set to:\n${correctedUrl}`, channelUrlMenu);
      await User.findOneAndUpdate({ userId }, { adminState: 'set_channel_url' });
    }
