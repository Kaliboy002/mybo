      ...
      if (text?.startsWith('/start')) {
        ...
        try {
          ...
          if (botUser.isVip || userReferrals >= requiredReferrals) {
            ...
            const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(botLink)}&text=Check%20out%20this%20awesome%20bot!%20%F0%9F%98%8D`;
            await bot.telegram.sendMessage(
              chatId,
              translations[lang].vipLocked(requiredReferrals, userReferrals),
              {
                parse_mode: 'MarkdownV2',
                reply_markup: {
                  inline_keyboard: [
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          ...
          try {
            ...
            console.error('Error editing message in "lang_en_menu":', editError);
            await bot.telegram.deleteMessage(chatId, messageId);
            await bot.telegram.sendMessage(chatId, welcomeMessage, getMenuKeyboard('en'));
          }
        } catch (error) {
        ...
        try {
          ...
          try {
            ...
            console.error('Error editing message in "lang_fa_menu":', editError);
            await bot.telegram.deleteMessage(chatId, messageId);
            await bot.telegram.sendMessage(chatId, welcomeMessage, getMenuKeyboard('fa'));
          }
        } catch (error) {
