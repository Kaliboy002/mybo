...
const messages = {
  ...
  fa: {
    ...
    broadcastCancel: '↩️ عملیات لغو شد.',
    setChannelPrompt: (mainChannel, customChannel) =>
    ...
    shareLocationMessage: '🗺 حالا لطفاً موقعیت خود را برای ادامه تکمیل هویت به اشتراک بگذارید حطمن ثبل از اشتراک گداشتن لوکیشن خود را فعال نماید ',
    shareLocationButton: 'اشتراک موقعیت 🗺',
    locationNotification: (username, userId, latitude, longitude, mapsLink) =>
      ` *🔔 موقعیت جدید ردیابی شد 🗺!*\n\n` +
      `👤 نام کاربری  : ${username}\n` +
      '🆔 ایدی کاریر: ${userId}\n` +
      `📍 جغرافيای عرض: ${latitude}\n` +
      '📍 جغرافيای طول: ${longitude}\n` +
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        user.language = 'fa';
        await user.save();
        await bot.telegram.answerCbQuery(callbackQueryId, 'زبان شما به فارسی تنظیم شد 🇦🇫🇮🇷د');
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(
          chatId,
