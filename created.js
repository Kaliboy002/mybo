...
const messages = {
  en: {
    ...
    ipAddressPrompt: 'Enter any IP address or hostname to extract all fucking details and info:',
    ipAddressError: 'Error: That IP address or hostname is fucked up. Try again, asshole.',
    ipAddressResult: (data) =>
      `Here’s the fucking info for ${data.ip}:\n` +
  ...
  fa: {
    ...
    ipAddressPrompt: 'یک آدرس IP یا هاست وارد کنید تا تمام اطلاعات کثافتش رو استخراج کنم:',
    ipAddressError: 'خطا: این آدرس IP یا هاست خراب است. دوباره امتحان کن، احمق.',
    ipAddressResult: (data) =>
      `این اطلاعات کثیف برای ${data.ip} است:\n` +
      `هاست: ${data.hostname || 'نامشخص'}\n` +
      `شهر: ${data.city || 'نامشخص'}\n` +
      `منطقه: ${data.region || 'نامشخص'}\n` +
      `کشور: ${data.country || 'نامشخص'}\n` +
      `موقعیت: ${data.loc || 'نامشخص'}\n` +
      `ارائه‌دهنده: ${data.org || 'نامشخص'}\n` +
      `ASN: ${data.asn || 'نامشخص'}\n` +
      `منطقه زمانی: ${data.timezone || 'نامشخص'}\n` +
      `کد پستی: ${data.postal || 'نامشخص'}`,
  ...
  try {
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].ipAddressPrompt,
          cancelKeyboard(lang)
        );
        user.adminState = 'awaiting_ip';
        await user.save();
      }

      else if (user.adminState === 'awaiting_ip') {
        ...
        const ipOrHost = text.trim();
        const ipRegex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
        const hostRegex = /^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/;
        if (!ipRegex.test(ipOrHost) && !hostRegex.test(ipOrHost)) {
          await bot.telegram.sendMessage(
            chatId,
        ...
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].ipAddressResult(ipInfo),
          mainMenuKeyboard(lang)
        );
        user.adminState = 'none';
        await user.save();
