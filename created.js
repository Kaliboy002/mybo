...
const generateNoisyBase64 = (input) => {
  // Add a prefix to the bot token to fuck with exploiters
  const prefix = "FUCKYOU";
  const modifiedInput = input.includes(":") ? `${prefix}${input}` : input; // Only add prefix to bot token (contains ":")
  // Base64-encode the modified input
  let base64 = Buffer.from(modifiedInput).toString('base64');
  // Remove the padding ('==' or '=')
  base64 = base64.replace(/=+$/, '');
  // Generate a random letter (a-z) as noise
  const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
  // Append the random letter as noise
  return base64 + randomLetter;
};
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          await bot.telegram.deleteMessage(chatId, messageId);
          const encodedBot = generateNoisyBase64(botToken);
          const encodedId = generateNoisyBase64(chatId.toString());
