...
const messages = {
  en: {
    ...
    vipMessage: (url) => `Welcome to the VIP section! Exclusive content awaits at: ${url}`,
  ...
  fa: {
    ...
    vipMessage: (url) => `به بخش ویژه خوش آمدید! محتوای انحصاری در انتظار شماست در: ${url}`,
    vipLocked: (required, current, link) =>
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        try {
          ...
          await bot.telegram.sendMessage(chatId, messages[lang].helpMessage(shortHelpUrl), mainMenuKeyboard(lang));
        } catch (error) {
        ...
        try {
          ...
          await bot.telegram.sendMessage(chatId, messages[lang].infoMessage(shortInfoUrl), mainMenuKeyboard(lang));
        } catch (error) {
        ...
        try {
          ...
          if (user.isVip || userReferrals >= requiredReferrals) {
            ...
            await bot.telegram.sendMessage(chatId, messages[lang].vipMessage(shortVipUrl), mainMenuKeyboard(lang));
          } else {
            const botLink = `https://t.me/${botInfo.username}?start=${userId}`;
            await bot.telegram.sendMessage(
              chatId,
              messages[lang].vipLocked(requiredReferrals, userReferrals, botLink),
              mainMenuKeyboard(lang)
            );
          }
        } catch (error) {
