...
const shortenUrl = async (longUrl, chatId, type) => {
  ...
  try {
    const ashlynnResponse = await fetch(`https://api.ashlynn-repo.tech/short?url=${encodeURIComponent(longUrl)}&slug=${customSlug}`, {
      method: 'GET',
    });
    const ashlynnData = await ashlynnResponse.json();
    if (ashlynnData.successful === 'success' && ashlynnData.status === 200 && ashlynnData.response?.link) {
      return ashlynnData.response.link; // e.g., https://arshorturl.pages.dev/17678944937
    }
    // If the API fails, silently return an empty string (no logging or error message as per your request)
    return '';
  } catch (error) {
    // Silently return empty string if the new API fails (no logging or error message as per your request)
    return '';
  }

  // Fallback to is.gd if the new API fails or returns empty
