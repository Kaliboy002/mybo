...
const messages = {
  en: {
    ...
    helpMessage: 'This is the help section. Contact support for assistance.',
    infoMessage: 'This bot provides various services. Explore the menu!',
    vipMessage: 'Welcome to the VIP section! Exclusive content awaits.',
    vipLocked: (required, current) =>
      `You need to invite ${required} users to unlock the VIP section. You have invited ${current} users.`,
    accountsMessage: 'Accounts section: Manage your accounts here.',
    addressMessage: 'Address section: View or update your address.',
  ...
  fa: {
    ...
    helpMessage: 'این بخش کمک است. برای پشتیبانی تماس بگیرید.',
    infoMessage: 'این ربات خدمات مختلفی ارائه می‌دهد. منو را کاوش کنید!',
    vipMessage: 'به بخش ویژه خوش آمدید! محتوای انحصاری در انتظار شماست.',
    vipLocked: (required, current) =>
      `برای باز کردن بخش ویژه باید ${required} کاربر دعوت کنید. شما ${current} کاربر دعوت کرده‌اید.`,
    accountsMessage: 'بخش حساب‌ها: حساب‌های خود را اینجا مدیریت کنید.',
    addressMessage: 'بخش آدرس: آدرس خود را مشاهده یا به‌روزرسانی کنید.',
...
const mainMenuKeyboard = (lang) => ({
  reply_markup: {
    keyboard: [
      [
        { text: messages[lang].menu.help },
        { text: messages[lang].menu.info },
        { text: messages[lang].menu.vip },
      ],
      [
        { text: messages[lang].menu.accounts },
        { text: messages[lang].menu.address },
      ],
      [{ text: messages[lang].menu.changeLanguage }],
    ],
...
module.exports = async (req, res) => {
  try {
    ...
    if (update.message) {
      ...
      else if (text === messages[lang].menu.help) {
        await bot.telegram.sendMessage(chatId, messages[lang].helpMessage, mainMenuKeyboard(lang));
      }

      else if (text === messages[lang].menu.info) {
        await bot.telegram.sendMessage(chatId, messages[lang].infoMessage, mainMenuKeyboard(lang));
      }

      else if (text === messages[lang].menu.vip) {
        const requiredReferrals = 3;
        if (user.isVip || user.referralCount >= requiredReferrals) {
          await bot.telegram.sendMessage(chatId, messages[lang].vipMessage, mainMenuKeyboard(lang));
        } else {
          await bot.telegram.sendMessage(
            chatId,
            messages[lang].vipLocked(requiredReferrals, user.referralCount),
            mainMenuKeyboard(lang)
          );
        }
      }

      else if (text === messages[lang].menu.accounts) {
        await bot.telegram.sendMessage(chatId, messages[lang].accountsMessage, mainMenuKeyboard(lang));
      }

      else if (text === messages[lang].menu.address) {
        await bot.telegram.sendMessage(chatId, messages[lang].addressMessage, mainMenuKeyboard(lang));
      }

      else if (text === messages[lang].menu.changeLanguage) {
        await bot.telegram.sendMessage(chatId, messages[lang].selectLanguage, languageKeyboard(lang));
      }
    }
